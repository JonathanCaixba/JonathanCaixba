# Hi there! I'm Jonathan Caixba 👋 

### Computer Systems Engineer | Java Full Stack Developer | AI-Augmented Dev

I'm a **Computer Systems Engineer** and **Java Full Stack Developer** specialized in building secure, high-performance web applications
My approach combines a solid academic background with modern intensive training from **Generation Mexico**.

I specialize in the **Java/Spring ecosystem**, leveraging **AI tools like GitHub Copilot** to accelerate development cycles and ensure technical excellence in every delivery.

---

### 🚀 Key Professional Highlights

**Process Optimization:** Developed an automated inventory system that reduced manual errors by 20% and slashed daily closing times from **1 hour to just 15 minutes** (75% efficiency gain).
*Security & Scalability:** Designed and implemented secure authentication modules and responsive architectures for E-commerce platforms like *AprenIdiomas*.
**Agile Mindset:** Experienced in **Scrum** environments, ensuring continuous integration and delivery (CI/CD) through Git/GitHub and automated deployments.

---

### 🛠 Tech Stack & Expertise

**Backend & Databases**
- ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white) ![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white) ![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)

**Frontend & Design**
- ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)

**Methodologies & AI Tools**
- ![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white) ![GitHub Copilot](https://img.shields.io/badge/GitHub_Copilot-000000?style=flat-square&logo=github-copilot&logoColor=white) ![Scrum](https://img.shields.io/badge/Scrum-000000?style=flat-square&logo=scrumalliance&logoColor=white)

---

### 🎓 Education & Growth
- **B.S. in Computer Systems Engineering** - Universidad Autónoma de Chiapas.
- **Java Full Stack Bootcamp** - Generation México (Graduated with honors).
- **Languages:** English (B1 Intermediate) | Spanish (Native). Actively improving for global teams.
- **Certifications in Progress:** "Java Masterclass: Job-Ready Development | Udemy (Tim Buchalka) | 130+ horas", Scrum Master.

---

### 📫 Let's Connect & Collaborate!
- **LinkedIn:** [Jonathan Caixba](https://www.linkedin.com/in/jonathan-caixba)
- **Email:** caixbajonathanfm@gmail.com
- **Location:** Tuxtla Gutiérrez, Chiapas, Mexico (Open to remote/relocation opportunities)

*"Turning complex problems into efficient, scalable solutions—one commit at a time."*
